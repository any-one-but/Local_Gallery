...
${makeCheckRow("Trim after first underscore", "Show only text before the first underscore in file names.", "opt_hideAfterFirstUnderscoreInFileNames", !!opt.hideAfterFirstUnderscoreInFileNames)}
    ...
    function getDisplayLeadPreviewForDir(dirNode, rotateKeyPrefix = "dir") {
      ...
      const recursivePool = getRecursivePreviewRecordsForDir(dirNode, PREVIEW_THUMB_PRIORITY_LIMIT, false);
    ...
    let PREVIEW_THUMB_REFRESH_RAF = 0;
    function schedulePreviewPaneRefreshFromThumbs() {
      if (PREVIEW_THUMB_REFRESH_RAF) return;
      PREVIEW_THUMB_REFRESH_RAF = requestAnimationFrame(() => {
        PREVIEW_THUMB_REFRESH_RAF = 0;
        renderPreviewPane(false);
      });
    }
    ...
    function ensureThumbUrl(rec) {
      ...
      const shouldGenerate = shouldGenerateImageThumb(rec, mode);
    ...
    async function prewarmVideoThumbsBeforeInitialRender() {
      if (!WS.root || !backgroundVideoThumbWorkEnabled()) return;
      const dirNode = getPreviewTargetDir();
      if (!dirNode) return;
      ...
      for (let i = 0; i < ids.length; i++) {
        ...
        if (!enqueueVideoThumb(rec, { deferDrain: true, priority: true })) continue;
        queued++;
        if (queued >= PREVIEW_VIDEO_PREWARM_LIMIT) break;
      }
      if (queued) drainVideoThumbQueue();
    }
    ...
    function kickVideoThumbsForPreview() {
      ...
      for (const id of ids) {
        ...
        queued++;
        if (queued >= PREVIEW_THUMB_PRIORITY_LIMIT) break;
      }
    ...
    function scheduleVideoThumbWorkspaceKick(delayMs = 0) {
      if (!backgroundVideoThumbWorkEnabled()) return;
      if (WS.videoThumbWorkspaceKickTimer) {
        try { clearTimeout(WS.videoThumbWorkspaceKickTimer); } catch {}
      }
      ...
      while (WS.videoThumbActive < MAX_VIDEO_THUMB_ACTIVE) {
        ...
        generateVideoThumb(rec).catch(() => {}).finally(() => {
          ...
          if (!WS.videoThumbPrewarmBlocking) {
            schedulePreviewPaneRefreshFromThumbs();
    ...
    function enqueueImageThumb(rec) {
      ...
      if (!shouldGenerateImageThumb(rec, mode)) return;
      if (!Array.isArray(WS.imageThumbQueue)) WS.imageThumbQueue = [];
    ...
    async function drainImageThumbQueue() {
      ...
      while (WS.imageThumbActive < 4 && WS.imageThumbQueue.length) {
        ...
        const mode = WS.meta && WS.meta.options ? String(WS.meta.options.imageThumbSize || "medium") : "medium";
        if (!shouldGenerateImageThumb(rec, mode)) continue;
    ...
    async function generateImageThumb(rec) {
      const mode = WS.meta && WS.meta.options ? String(WS.meta.options.imageThumbSize || "medium") : "medium";
      if (!shouldGenerateImageThumb(rec, mode)) {
        if (rec.thumbUrl && rec.thumbMode && rec.thumbMode !== "high") {
          try { URL.revokeObjectURL(rec.thumbUrl); } catch {}
          rec.thumbUrl = null;
        }
        rec.thumbMode = (mode === "high") ? "high" : null;
        return;
      }
