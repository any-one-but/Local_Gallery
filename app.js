    ...
    const THUMB_DISK_CACHE_SCHEMA_VERSION = "nav-v2";
    let THUMB_DISK_SCOPE_KEY = "";
    const THUMB_DISK_READ_PROMISES = new Map();
    const THUMB_DISK_WRITE_PROMISES = new Map();
    const DIR_NAV_MANIFESTS = new Map();
    const NAV_THUMB_READY_PROMISES = new Map();
    let NAVIGATION_PREP_TOKEN = 0;
    const HOT_NAV_DIR_PATHS = new Map();
    const PINNED_HOT_NAV_DIR_PATHS = new Set();
    const PINNED_HOT_NAV_DECODE_CACHE = new Map();
    let HOT_NAV_CACHE_TOKEN = 0;
    let HOT_NAV_CACHE_TIMER = 0;
    const HOT_NAV_MAX_DIRS = 24;
    const HOT_NAV_NEARBY_SIBLING_RADIUS = 3;
    ...
    */
    ...
    function resetWorkspace() {
      revokeAllObjectURLs();
      THUMB_DISK_SCOPE_KEY = "";
      THUMB_DISK_READ_PROMISES.clear();
      THUMB_DISK_WRITE_PROMISES.clear();
      NAV_THUMB_READY_PROMISES.clear();
      NAVIGATION_PREP_TOKEN = 0;
      HOT_NAV_DIR_PATHS.clear();
      PINNED_HOT_NAV_DIR_PATHS.clear();
      PINNED_HOT_NAV_DECODE_CACHE.clear();
      HOT_NAV_CACHE_TOKEN = 0;
    ...
    window.addEventListener("resize", () => {
      ...
      updateGridModeListTopInset();
      scheduleDirectoriesVirtualViewportRefresh();
...
${makeCheckRow("Trim after first underscore", "Show only text before the first underscore in file names.", "opt_hideAfterFirstUnderscoreInFileNames", !!opt.hideAfterFirstUnderscoreInFileNames)}
    ...
    function directoryNavManifestKey(dirNode) {
      if (!dirNode) return "";
      return String(dirNode.path || "__root__");
    }

    function getDirectoryNavManifest(dirNode) {
      if (!dirNode) return null;
      const key = directoryNavManifestKey(dirNode);
      if (DIR_NAV_MANIFESTS.has(key)) return DIR_NAV_MANIFESTS.get(key) || null;

      const fileIds = getOrderedFileIdsForDir(dirNode, false).map((id) => String(id || "")).filter(Boolean);
      const childDirs = getChildDirsForNode(dirNode);
      const childDirPaths = [];
      const childLeadRecordIds = [];
      for (let i = 0; i < childDirs.length; i++) {
        const child = childDirs[i];
        if (!child) continue;
        childDirPaths.push(String(child.path || ""));
        const childManifest = getDirectoryNavManifest(child);
        const childLeadId = String(childManifest && childManifest.leadRecordId || "");
        if (childLeadId) childLeadRecordIds.push(childLeadId);
      }

      const presetRec = getPresetPreviewRecordForDir(dirNode);
      const directRec = getFirstDirectPreviewRecordForDir(dirNode);
      const leadRecordId = String(
        (presetRec && presetRec.id)
        || (directRec && directRec.id)
        || (childLeadRecordIds[0] || "")
      );
      const navRecordIds = [];
      const seen = new Set();
      const pushId = (id) => {
        ...
        seen.add(normalized);
        navRecordIds.push(normalized);
      };
      for (let i = 0; i < fileIds.length; i++) pushId(fileIds[i]);
      for (let i = 0; i < childLeadRecordIds.length; i++) pushId(childLeadRecordIds[i]);

      const manifest = {
        key,
        path: String(dirNode.path || ""),
        fileIds,
        childDirPaths,
        childLeadRecordIds,
        leadRecordId,
      ...
      DIR_NAV_MANIFESTS.set(key, manifest);
      return manifest;
    }

    function primeDirectoryNavManifests(rootNode = null) {
      const startNode = rootNode || WS.root;
      if (!startNode) return;
      getDirectoryNavManifest(startNode);
    }

    function manifestPathKey(pathValue) {
      const path = String(pathValue || "");
      return path || "__root__";
    }

    function collectManifestNavigationRecordsForPaths(paths) {
      const out = [];
      const seen = new Set();
      const list = Array.isArray(paths) ? paths : [];
      for (let i = 0; i < list.length; i++) {
        const key = manifestPathKey(list[i]);
        const manifest = DIR_NAV_MANIFESTS.get(key);
    ...
    function collectWorkspaceNavigationRecords(rootNode = null) {
      const startNode = rootNode || WS.root;
      if (!startNode) return [];
      primeDirectoryNavManifests(startNode);
      const rootPath = String(startNode.path || "");
      const paths = [];
      for (const manifest of DIR_NAV_MANIFESTS.values()) {
        const path = String(manifest && manifest.path || "");
        if (!rootPath || path === rootPath || path.startsWith(rootPath + "/")) paths.push(path);
      }
      return collectManifestNavigationRecordsForPaths(paths);
    }
    ...
    async function setDirectoriesSelection(idx, opts = null) {
    ...
    async function returnToSearchResults() {
      ...
      const ready = await prepareNavigationSnapshot({
        ...
        preview: {
          ...
          dirNode: WS.preview.dirNode,
          fileId: WS.preview.fileId
        }
    ...
    function thumbDiskCacheAvailable() {
      const electronApi = getElectronApi();
      return !!(electronApi
        && typeof electronApi.readThumbCache === "function"
        && typeof electronApi.writeThumbCache === "function");
    }

    function currentThumbDiskScopeKey() {
      if (!thumbDiskCacheAvailable()) return "";
      if (THUMB_DISK_SCOPE_KEY) return THUMB_DISK_SCOPE_KEY;
      const absRoot = resolveAbsoluteDirectoryPath("");
      if (absRoot) {
        THUMB_DISK_SCOPE_KEY = `root:${absRoot}`;
        return THUMB_DISK_SCOPE_KEY;
      }
      const storageKey = String(WS.meta && WS.meta.storageKey || "").trim();
      if (storageKey) {
        THUMB_DISK_SCOPE_KEY = `storage:${storageKey}`;
        return THUMB_DISK_SCOPE_KEY;
      }
      return "";
    }

    function thumbDiskRecordIdentity(rec) {
      if (!rec) return "";
      const nativePath = String(rec.nativePath || getNativePathForFile(rec.file) || "");
      const relPath = String(rec.relPath || "");
      const size = Number.isFinite(Number(rec.size)) ? Number(rec.size) : Number(rec.file && rec.file.size || 0);
      const lastModified = Number.isFinite(Number(rec.lastModified)) ? Number(rec.lastModified) : Number(rec.file && rec.file.lastModified || 0);
      return `${String(rec.type || "")}|${nativePath || relPath}|${size}|${lastModified}`;
    }

    function serializeThumbDiskCrop(crop) {
    ...
    function currentThumbProcessingCacheKey() {
      return THUMB_FILTER_KEY || buildThumbFilterKey();
    }

    function imageThumbDiskCacheKey(rec, mode) {
      if (!rec || rec.type !== "image") return "";
      const normalizedMode = String(mode || "medium");
      if (normalizedMode === "high" && !thumbFiltersActive(rec)) return "";
      return [
        THUMB_DISK_CACHE_SCHEMA_VERSION,
        "image",
        thumbDiskRecordIdentity(rec),
        `mode:${normalizedMode}`,
        `width:${imageThumbWidthForOption()}`,
        "variant:nav-neutral"
      ].join("###");
    }

    function videoThumbDiskCacheKey(rec, mode) {
      ...
      return [
        ...
        thumbDiskRecordIdentity(rec),
        `mode:${normalizedMode}`,
        `width:${videoThumbWidthForOption()}`,
        `frame:${String(metaGetVideoThumbnailTimeForRecord(rec) ?? "auto")}`,
        `crop:${serializeThumbDiskCrop(getVideoCropForRecord(rec))}`,
        "variant:nav-neutral"
      ].join("###");
    }

    async function readThumbDiskCacheUrl(scopeKey, cacheKey) {
      if (!scopeKey || !cacheKey || !thumbDiskCacheAvailable()) return "";
      const promiseKey = `${scopeKey}###${cacheKey}`;
      if (THUMB_DISK_READ_PROMISES.has(promiseKey)) {
        return THUMB_DISK_READ_PROMISES.get(promiseKey);
      }
      const electronApi = getElectronApi();
      const loadPromise = (async () => {
        try {
          const url = await electronApi.readThumbCache({ scopeKey, cacheKey, ext: "jpg" });
          return String(url || "");
        } catch {
          return "";
        } finally {
          THUMB_DISK_READ_PROMISES.delete(promiseKey);
        }
      })();
      THUMB_DISK_READ_PROMISES.set(promiseKey, loadPromise);
      return loadPromise;
    }

    async function writeThumbDiskCacheUrl(scopeKey, cacheKey, blob) {
      if (!scopeKey || !cacheKey || !blob || !thumbDiskCacheAvailable()) return "";
      const promiseKey = `${scopeKey}###${cacheKey}`;
      if (THUMB_DISK_WRITE_PROMISES.has(promiseKey)) {
        return THUMB_DISK_WRITE_PROMISES.get(promiseKey);
      }
      const electronApi = getElectronApi();
      const writePromise = (async () => {
        try {
          const bytes = new Uint8Array(await blob.arrayBuffer());
          const url = await electronApi.writeThumbCache({ scopeKey, cacheKey, ext: "jpg", bytes });
          return String(url || "");
        } catch {
          return "";
        } finally {
          THUMB_DISK_WRITE_PROMISES.delete(promiseKey);
        }
      })();
      THUMB_DISK_WRITE_PROMISES.set(promiseKey, writePromise);
      return writePromise;
    }

    async function hydrateImageThumbFromDiskCache(rec, mode) {
      if (!rec || rec.type !== "image") return false;
      if (rec.thumbUrl && rec.thumbMode === mode) return true;
      const scopeKey = currentThumbDiskScopeKey();
      const cacheKey = imageThumbDiskCacheKey(rec, mode);
      if (!scopeKey || !cacheKey) return false;
      const cachedUrl = await readThumbDiskCacheUrl(scopeKey, cacheKey);
      if (!cachedUrl) return false;
      if (rec.thumbUrl && rec.thumbMode && rec.thumbMode !== "high") {
        try { URL.revokeObjectURL(rec.thumbUrl); } catch {}
      }
      rec.thumbUrl = cachedUrl;
      rec.thumbMode = mode;
      return true;
    }

    async function hydrateVideoThumbFromDiskCache(rec, mode) {
      if (!rec || rec.type !== "video") return false;
      if (rec.videoThumbUrl && rec.videoThumbMode === mode) return true;
      const scopeKey = currentThumbDiskScopeKey();
      const cacheKey = videoThumbDiskCacheKey(rec, mode);
      if (!scopeKey || !cacheKey) return false;
      const cachedUrl = await readThumbDiskCacheUrl(scopeKey, cacheKey);
      if (!cachedUrl) return false;
      if (rec.videoThumbUrl) {
      ...
      rec.videoThumbUrl = cachedUrl;
      rec.videoThumbMode = mode;
      return true;
    }

    async function persistImageThumbToDiskCache(rec, mode, blob) {
      const scopeKey = currentThumbDiskScopeKey();
      const cacheKey = imageThumbDiskCacheKey(rec, mode);
      if (!scopeKey || !cacheKey) return "";
      return writeThumbDiskCacheUrl(scopeKey, cacheKey, blob);
    }

    async function persistVideoThumbToDiskCache(rec, mode, blob) {
      const scopeKey = currentThumbDiskScopeKey();
      const cacheKey = videoThumbDiskCacheKey(rec, mode);
      if (!scopeKey || !cacheKey) return "";
      return writeThumbDiskCacheUrl(scopeKey, cacheKey, blob);
    ...
    function currentImageThumbMode() {
      return WS.meta && WS.meta.options ? String(WS.meta.options.imageThumbSize || "medium") : "medium";
    }
    ...
    function previewStateForEntry(entry) {
      ...
      if (entry.kind === "tag") {
        ...
        return node
          ? { kind: "dir", dirNode: node, fileId: null }
    ...
    function applyPreviewState(state) {
      ...
      WS.preview.dirNode = next.dirNode || null;
      WS.preview.fileId = next.fileId || null;
    }
    ...
    async function ensureNavigationThumbReady(rec) {
      if (!rec || !rec.id) return false;
      if (rec.type === "video") {
        const mode = currentVideoThumbMode();
        if (rec.videoThumbUrl && rec.videoThumbMode === mode) return true;
        const key = `video###${String(rec.id || "")}###${videoThumbDiskCacheKey(rec, mode)}`;
        if (NAV_THUMB_READY_PROMISES.has(key)) return NAV_THUMB_READY_PROMISES.get(key);
        const promise = generateVideoThumb(rec).then(() => !!(rec.videoThumbUrl && rec.videoThumbMode === mode)).catch(() => false).finally(() => {
          NAV_THUMB_READY_PROMISES.delete(key);
        });
        NAV_THUMB_READY_PROMISES.set(key, promise);
        return promise;
      }
      if (rec.type === "image") {
        ...
        if (NAV_THUMB_READY_PROMISES.has(key)) return NAV_THUMB_READY_PROMISES.get(key);
        const promise = generateImageThumb(rec).then(() => !!(rec.thumbUrl && rec.thumbMode === mode)).catch(() => false).finally(() => {
          NAV_THUMB_READY_PROMISES.delete(key);
        });
        NAV_THUMB_READY_PROMISES.set(key, promise);
        return promise;
    ...
    function collectNearbyDirectoryPaths(snapshot) {
      const out = [];
      const seen = new Set();
      const pushPath = (pathValue) => {
        const path = String(pathValue || "");
        if (seen.has(path)) return;
        seen.add(path);
        out.push(path);
      };
      const pushDir = (dirNode) => {
        if (!dirNode) return;
        pushPath(dirNode.path || "");
      };
      const pushChildDirs = (dirNode, limit = HOT_NAV_CHILD_LIMIT) => {
        if (!dirNode) return;
        const children = getChildDirsForNode(dirNode);
        for (let i = 0; i < children.length && i < limit; i++) pushDir(children[i]);
      };

      const navDir = snapshot && snapshot.dirNode ? snapshot.dirNode : null;
      const previewDir = snapshot && snapshot.preview && snapshot.preview.kind === "dir" ? snapshot.preview.dirNode : null;
      pushDir(navDir);
      pushDir(previewDir);
      if (navDir && navDir.parent) pushDir(navDir.parent);
      if (previewDir && previewDir.parent) pushDir(previewDir.parent);

      const selectedIndex = Number(WS.nav.selectedIndex || 0);
      const start = Math.max(0, selectedIndex - HOT_NAV_NEARBY_SIBLING_RADIUS);
      const end = Math.min(WS.nav.entries.length - 1, selectedIndex + HOT_NAV_NEARBY_SIBLING_RADIUS);
      ...
      const selectedEntry = WS.nav.entries[selectedIndex] || null;
      if (selectedEntry && selectedEntry.kind === "dir" && selectedEntry.node) {
        pushDir(selectedEntry.node);
        pushChildDirs(selectedEntry.node, HOT_NAV_CHILD_LIMIT);
      }

      pushChildDirs(navDir, HOT_NAV_CHILD_LIMIT);
      pushChildDirs(previewDir, Math.max(2, Math.floor(HOT_NAV_CHILD_LIMIT / 2)));
      return out;
    }

    function collectPinnedHotDirectoryPaths(rootNode = null) {
      const startNode = rootNode || WS.root;
      if (!startNode) return [];
      const scoreMemo = new Map();
      const scoreDir = (dirNode) => {
        if (!dirNode) return 0;
        const key = String(dirNode.path || "");
        if (scoreMemo.has(key)) return scoreMemo.get(key) || 0;
        let score = getOrderedFileIdsForDir(dirNode, false).length + getChildDirsForNode(dirNode).length;
        const children = getChildDirsForNode(dirNode);
        for (let i = 0; i < children.length; i++) score += scoreDir(children[i]);
      ...
      while (stack.length) {
        ...
        if (!dirNode) continue;
        allDirs.push(dirNode);
        const children = getChildDirsForNode(dirNode);
        for (let i = children.length - 1; i >= 0; i--) stack.push(children[i]);
      }
      allDirs.sort((a, b) => scoreDir(b) - scoreDir(a));
      const out = [];
      const seen = new Set();
      const pushPath = (dirNode) => {
        if (!dirNode) return;
      ...
      pushPath(startNode);
      for (let i = 0; i < allDirs.length && out.length < (HOT_NAV_LARGE_DIR_LIMIT + 1); i++) {
        pushPath(allDirs[i]);
      }
      return out;
    }

    function syncPinnedHotNavigationDirectories(rootNode = null) {
      PINNED_HOT_NAV_DIR_PATHS.clear();
      const paths = collectPinnedHotDirectoryPaths(rootNode || WS.root);
      for (let i = 0; i < paths.length; i++) PINNED_HOT_NAV_DIR_PATHS.add(String(paths[i] || ""));
      return paths;
    }

    function collectEffectiveHotDirectoryPaths(snapshot) {
      const paths = [];
      const seen = new Set();
      const pushPath = (pathValue) => {
        const path = String(pathValue || "");
      ...
      PINNED_HOT_NAV_DIR_PATHS.forEach((path) => pushPath(path));
      const nearby = collectNearbyDirectoryPaths(snapshot);
      for (let i = 0; i < nearby.length; i++) pushPath(nearby[i]);
      return paths;
    }

    function rememberPinnedHotDecode(img, key) {
      if (!img || !key) return;
      if (PINNED_HOT_NAV_DECODE_CACHE.has(key)) PINNED_HOT_NAV_DECODE_CACHE.delete(key);
      PINNED_HOT_NAV_DECODE_CACHE.set(key, img);
      while (PINNED_HOT_NAV_DECODE_CACHE.size > PINNED_HOT_NAV_DECODE_LIMIT) {
        const oldest = PINNED_HOT_NAV_DECODE_CACHE.keys().next();
        if (oldest && !oldest.done) PINNED_HOT_NAV_DECODE_CACHE.delete(oldest.value);
        else break;
      }
    }

    async function warmPinnedNavigationThumbDecode(rec) {
      if (!rec || !rec.id) return;
      const src = rec.type === "video"
        ? String(rec.videoThumbUrl || "")
        : String(rec.thumbUrl || "");
      if (!src) return;
      const key = `${String(rec.id || "")}###${src}`;
      if (PINNED_HOT_NAV_DECODE_CACHE.has(key)) {
        const img = PINNED_HOT_NAV_DECODE_CACHE.get(key);
        PINNED_HOT_NAV_DECODE_CACHE.delete(key);
        PINNED_HOT_NAV_DECODE_CACHE.set(key, img);
        return;
      }
      const img = new Image();
      ...
      rememberPinnedHotDecode(img, key);
    }

    function touchHotNavigationDirectoryPaths(paths) {
      const list = Array.isArray(paths) ? paths : [];
      for (let i = 0; i < list.length; i++) {
        const path = String(list[i] || "");
        if (HOT_NAV_DIR_PATHS.has(path)) HOT_NAV_DIR_PATHS.delete(path);
        HOT_NAV_DIR_PATHS.set(path, Date.now());
      }
      while (HOT_NAV_DIR_PATHS.size > HOT_NAV_MAX_DIRS) {
        const oldest = HOT_NAV_DIR_PATHS.keys().next();
        if (oldest && !oldest.done) HOT_NAV_DIR_PATHS.delete(oldest.value);
        else break;
      }
    }

    function releaseColdNavigationThumbs() {
      return;
    }
    ...
    async function warmNearbyDirectoriesInRam(snapshot) {
      ...
      const records = collectManifestNavigationRecordsForPaths(paths);
      if (records.length) await runWithConcurrency(records, navigationThumbWorkerLimit("background"), ensureNavigationThumbReady);
      const pinnedRecords = records.filter((rec) => {
        if (!rec) return false;
        return PINNED_HOT_NAV_DIR_PATHS.has(String(rec.dirPath || ""));
      });
      if (pinnedRecords.length) {
        await runWithConcurrency(pinnedRecords, navigationThumbWorkerLimit("background"), warmPinnedNavigationThumbDecode);
      }
    ...
    function scheduleNearbyDirectoryRamWarm(snapshot, delayMs = 0) {
      ...
      HOT_NAV_CACHE_TIMER = setTimeout(() => {
        ...
        warmNearbyDirectoriesInRam(snapshot).catch(() => {});
      }, waitMs);
    }

    function navigationSnapshotPendingRecords(snapshot) {
      const records = collectNavigationSnapshotRecords(snapshot);
    ...
    async function ensureNavigationSnapshotReady(snapshot) {
      const records = navigationSnapshotPendingRecords(snapshot);
      if (!records.length) return;
    ...
    async function prepareNavigationSnapshot(snapshot, label = "Preparing thumbnails...") {
      ...
      const token = ++NAVIGATION_PREP_TOKEN;
      showBusyOverlay(label);
      try {
        await runWithConcurrency(pending, navigationThumbWorkerLimit("blocking"), ensureNavigationThumbReady);
      } finally {
        if (token === NAVIGATION_PREP_TOKEN) hideBusyOverlay();
      }
      if (token === NAVIGATION_PREP_TOKEN) scheduleNearbyDirectoryRamWarm(snapshot);
    ...
    async function restoreDirHistoryEntry(entry) {
    ...
    function currentThumbWorkerCpuBudget() {
      const reported = Number((typeof navigator !== "undefined" && navigator && navigator.hardwareConcurrency) || 0);
      const cores = Number.isFinite(reported) && reported > 0 ? reported : 4;
      return Math.max(2, Math.min(12, Math.round(cores)));
    }

    function navigationThumbWorkerLimit(kind = "blocking") {
      const cores = currentThumbWorkerCpuBudget();
      if (kind === "startup") return Math.max(4, Math.min(8, cores));
      if (kind === "background") return Math.max(2, Math.min(4, Math.floor(cores / 2)));
      return Math.max(3, Math.min(6, Math.floor((cores * 3) / 4)));
    }

    function videoThumbPoolLimits() {
      const cores = currentThumbWorkerCpuBudget();
      if (WS.videoThumbPrewarmBlocking) {
        const active = Math.max(4, Math.min(8, cores));
        return { active, background: 0 };
      }
      const active = Math.max(2, Math.min(5, Math.floor((cores + 2) / 2)));
      const background = Math.max(1, Math.min(2, Math.floor(cores / 4)));
      return { active, background };
    }

    function imageThumbPoolLimit() {
      const cores = currentThumbWorkerCpuBudget();
      return Math.max(2, Math.min(5, Math.floor((cores + 3) / 3)));
    }

    function shouldVirtualizeDirectoriesPane(gridModeActive) {
      if (!gridModeActive) return false;
      if ((WS.nav.entries || []).length < 220) return false;
      for (let i = 0; i < WS.nav.entries.length; i++) {
        const entry = WS.nav.entries[i];
        if (!entry) continue;
        if (entry.kind === "tag" || entry.kind === "up") return false;
      }
      if (WS.view.dirActionMenuPath || WS.view.fileActionMenuId || WS.view.bulkActionMenuOpen) return false;
      if (TAG_ENTRY_RENAME_STATE || RENAME_EDIT_PATH || RENAME_EDIT_FILE_ID) return false;
      return true;
    }
    ...
    function measureDirectoriesGridVirtualMetrics() {
      if (!directoriesListEl) return null;
      const cs = getComputedStyle(directoriesListEl);
      const gap = Math.max(0, parseFloat(cs.rowGap || cs.gap || "0") || 0);
      const card = Math.max(80, parseFloat(cs.getPropertyValue("--grid-card-size") || "0") || 0);
      const colsRaw = parseInt(cs.getPropertyValue("--grid-cols"), 10);
      const cols = Math.max(1, Number.isFinite(colsRaw) ? colsRaw : 1);
      const rowHeight = Math.max(96, card) + gap;
      return { gap, card, cols, rowHeight };
    }

    function computeDirectoriesVirtualWindow(entryCount, metrics, suppressSelectionReveal = false) {
      if (!directoriesListEl || !metrics || !entryCount) return null;
      const totalRows = Math.max(1, Math.ceil(entryCount / Math.max(1, metrics.cols)));
      const scrollTop = Math.max(0, Number(directoriesListEl.scrollTop) || 0);
      const clientHeight = Math.max(1, Number(directoriesListEl.clientHeight) || 1);
      let anchorRow = Math.max(0, Math.floor(scrollTop / Math.max(1, metrics.rowHeight)));
      const viewportRows = Math.max(1, Math.ceil((clientHeight + metrics.gap) / Math.max(1, metrics.rowHeight)));
      const selectedRow = Math.max(0, Math.floor(Math.max(0, Number(WS.nav.selectedIndex) || 0) / Math.max(1, metrics.cols)));
      if (!suppressSelectionReveal && (
        WS.view.pendingDirScroll === "center-selected"
        || WS.view.pendingDirScroll === "grid-nearest"
        || selectedRow < anchorRow
        || selectedRow > (anchorRow + viewportRows)
      )) {
        anchorRow = Math.max(0, selectedRow - Math.floor(viewportRows / 2));
      }
      const overscanRows = 2;
      const startRow = Math.max(0, anchorRow - overscanRows);
      const endRow = Math.min(totalRows - 1, anchorRow + viewportRows + overscanRows);
      return {
        startIndex: startRow * metrics.cols,
        endIndexExclusive: Math.min(entryCount, (endRow + 1) * metrics.cols),
        topSpacerHeight: startRow * metrics.rowHeight,
        bottomSpacerHeight: Math.max(0, (totalRows - endRow - 1) * metrics.rowHeight)
      };
    }

    function appendVirtualFlexSpacer(frag, heightPx) {
      const height = Math.max(0, Math.round(Number(heightPx) || 0));
      if (!frag || height <= 0) return;
      const spacer = document.createElement("div");
      spacer.className = "virtualFlexSpacer";
      spacer.setAttribute("aria-hidden", "true");
      spacer.style.height = `${height}px`;
      frag.appendChild(spacer);
    }

    function scheduleDirectoriesVirtualViewportRefresh() {
      if (!directoriesListEl || directoriesListEl.dataset.virtualized !== "1") return;
      if (DIRECTORIES_VIRTUAL_SCROLL_RAF) return;
      DIRECTORIES_VIRTUAL_SCROLL_RAF = requestAnimationFrame(() => {
        DIRECTORIES_VIRTUAL_SCROLL_RAF = 0;
        if (!directoriesListEl || directoriesListEl.dataset.virtualized !== "1") return;
        renderDirectoriesPane(true, { suppressSelectionReveal: true });
      });
    }

    function renderDirectoriesPane(keepScroll = false, opts = null) {
      ...
      const virtualizeGrid = shouldVirtualizeDirectoriesPane(gridModeActive);
      const virtualMetrics = virtualizeGrid ? measureDirectoriesGridVirtualMetrics() : null;
      const virtualWindow = virtualizeGrid ? computeDirectoriesVirtualWindow(WS.nav.entries.length, virtualMetrics, suppressSelectionReveal) : null;
      const entryStart = virtualWindow ? Math.max(0, virtualWindow.startIndex) : 0;
      const entryEndExclusive = virtualWindow ? Math.max(entryStart, virtualWindow.endIndexExclusive) : WS.nav.entries.length;
      directoriesListEl.dataset.virtualized = virtualWindow ? "1" : "0";
      appendVirtualFlexSpacer(frag, virtualWindow ? virtualWindow.topSpacerHeight : 0);
      ...
      appendVirtualFlexSpacer(frag, virtualWindow ? virtualWindow.bottomSpacerHeight : 0);
    ...
    function clearPreviewFileVirtualizers() {
      ACTIVE_PREVIEW_FILE_VIRTUALIZERS = [];
    }

    function shouldVirtualizePreviewFilesGrid(ids, useFitInsideJustified, dirNode) {
      if (!Array.isArray(ids) || ids.length < 220) return false;
      if (useFitInsideJustified) return false;
      if (dirNode && canReorderFilesInPreviewDir(dirNode)) return false;
      return true;
    }

    function measurePreviewFilesGridMetrics(grid, useSquareMediaCards = false) {
      if (!grid) return null;
      const cs = getComputedStyle(grid);
      const gap = Math.max(0, parseFloat(cs.rowGap || cs.gap || "0") || 0);
      const minCard = Math.max(96, parseFloat(cs.getPropertyValue("--file-grid-min") || "160") * (parseFloat(cs.getPropertyValue("--thumb-scale-media") || "1") || 1));
      const width = Math.max(1, Number(grid.clientWidth) || 1);
      const cols = Math.max(1, Math.floor((width + gap) / Math.max(1, minCard + gap)));
      const colWidth = Math.max(96, (width - (gap * Math.max(0, cols - 1))) / cols);
      const rowHeight = useSquareMediaCards
        ? Math.max(120, Math.round(colWidth + 48))
        : Math.max(150, Math.round((colWidth * 0.75) + 74));
      return { gap, cols, colWidth, rowHeight };
    }
    ...
    function computePreviewFilesVirtualWindow(state) {
      if (!state || !state.grid || !previewBodyEl) return null;
      const metrics = measurePreviewFilesGridMetrics(state.grid, state.useSquareMediaCards);
      if (!metrics) return null;
      const totalRows = Math.max(1, Math.ceil(state.ids.length / Math.max(1, metrics.cols)));
      const gridTop = offsetTopWithinAncestor(state.grid, previewBodyEl);
      const viewTop = Math.max(0, (previewBodyEl.scrollTop || 0) - gridTop);
      const viewportRows = Math.max(1, Math.ceil(((previewBodyEl.clientHeight || 1) + metrics.gap) / Math.max(1, metrics.rowHeight)));
      const startRow = Math.max(0, Math.floor(viewTop / Math.max(1, metrics.rowHeight)) - 2);
      const endRow = Math.min(totalRows - 1, startRow + viewportRows + 4);
      return {
        metrics,
        startIndex: startRow * metrics.cols,
        endIndexExclusive: Math.min(state.ids.length, (endRow + 1) * metrics.cols),
        topSpacerHeight: startRow * metrics.rowHeight,
        bottomSpacerHeight: Math.max(0, (totalRows - endRow - 1) * metrics.rowHeight)
      };
    }

    function appendVirtualGridSpacer(grid, heightPx) {
      const height = Math.max(0, Math.round(Number(heightPx) || 0));
      if (!grid || height <= 0) return;
      const spacer = document.createElement("div");
      spacer.className = "virtualGridSpacer";
      spacer.setAttribute("aria-hidden", "true");
      spacer.style.height = `${height}px`;
      grid.appendChild(spacer);
    }

    function updateVirtualPreviewFilesGrid(state, force = false) {
      if (!state || !state.grid || !state.grid.isConnected) return;
      if (state.renderToken !== PREVIEW_FILES_RENDER_TOKEN) return;
      const windowInfo = computePreviewFilesVirtualWindow(state);
      if (!windowInfo) return;
      if (!force
        && state.currentStart === windowInfo.startIndex
        && state.currentEnd === windowInfo.endIndexExclusive) {
        return;
      }
      state.currentStart = windowInfo.startIndex;
      state.currentEnd = windowInfo.endIndexExclusive;
      state.grid.innerHTML = "";
      appendVirtualGridSpacer(state.grid, windowInfo.topSpacerHeight);
      const frag = document.createDocumentFragment();
      for (let i = windowInfo.startIndex; i < windowInfo.endIndexExclusive; i++) {
        const id = state.ids[i];
        const rec = WS.fileById.get(id);
        if (!rec) continue;
        const shouldAnimateCard = !!state.animate && (i - windowInfo.startIndex) < 96;
        frag.appendChild(makePreviewFileCard(rec, shouldAnimateCard, state.dirNode, state.ids, state.useFitInsideJustified, state.useSquareMediaCards));
      }
    ...
    function registerPreviewFileVirtualizer(state) {
      if (!state) return;
      ACTIVE_PREVIEW_FILE_VIRTUALIZERS.push(state);
      updateVirtualPreviewFilesGrid(state, true);
    }

    function schedulePreviewVirtualGridRefresh() {
      if (!ACTIVE_PREVIEW_FILE_VIRTUALIZERS.length) return;
      if (PREVIEW_VIRTUAL_SCROLL_RAF) return;
      PREVIEW_VIRTUAL_SCROLL_RAF = requestAnimationFrame(() => {
        PREVIEW_VIRTUAL_SCROLL_RAF = 0;
        const list = ACTIVE_PREVIEW_FILE_VIRTUALIZERS.slice();
        for (let i = 0; i < list.length; i++) updateVirtualPreviewFilesGrid(list[i], false);
      });
    }
    ...
    function renderFilesGrid(ids, container, animate, dirNode, renderToken = PREVIEW_FILES_RENDER_TOKEN) {
      ...
      if (shouldVirtualizePreviewFilesGrid(ids, useFitInsideJustified, dirNode)) {
        registerPreviewFileVirtualizer({
          grid,
          ids: ids.slice(),
          dirNode,
          animate: !!animate,
    ...
    async function drainVideoThumbQueue() {
      const poolLimits = videoThumbPoolLimits();
      const MAX_VIDEO_THUMB_ACTIVE = poolLimits.active;
      const MAX_VIDEO_THUMB_BACKGROUND_ACTIVE = poolLimits.background;
      if (!Array.isArray(WS.videoThumbPriorityQueue)) WS.videoThumbPriorityQueue = [];
      if (!Array.isArray(WS.videoThumbQueue)) WS.videoThumbQueue = [];
      if (!(WS.videoThumbQueuedIds instanceof Set)) WS.videoThumbQueuedIds = new Set();
      if (!(WS.videoThumbInFlightIds instanceof Set)) WS.videoThumbInFlightIds = new Set();
    ...
    async function generateVideoThumb(rec) {
      const mode = currentVideoThumbMode();
      if (await hydrateVideoThumbFromDiskCache(rec, mode)) return;
      ...
      try {
        ...
        for (let i = 0; i < seekTimes.length; i++) {
          ...
          renderFilteredToCanvas(ctx, v, v.videoWidth || w, v.videoHeight || h, w, h, "off", true, rec);
        ...
        if (!blob) return;
        const cachedUrl = await persistVideoThumbToDiskCache(rec, mode, blob);
        rec.videoThumbUrl = cachedUrl || URL.createObjectURL(blob);
      } finally {
        try { v.pause(); } catch {}
    ...
    function enqueueImageThumb(rec) {
      ...
      if (!(WS.imageThumbQueuedIds instanceof Set)) WS.imageThumbQueuedIds = new Set();
      if (!(WS.imageThumbInFlightIds instanceof Set)) WS.imageThumbInFlightIds = new Set();
      if (WS.imageThumbQueuedIds.has(id) || WS.imageThumbInFlightIds.has(id)) return;
      WS.imageThumbQueuedIds.add(id);
      WS.imageThumbQueue.push(id);
      drainImageThumbQueue();
    }

    async function drainImageThumbQueue() {
      const maxActive = imageThumbPoolLimit();
      if (!(WS.imageThumbQueuedIds instanceof Set)) WS.imageThumbQueuedIds = new Set();
      if (!(WS.imageThumbInFlightIds instanceof Set)) WS.imageThumbInFlightIds = new Set();
      if (WS.imageThumbActive >= maxActive) return;
      while (WS.imageThumbActive < maxActive && WS.imageThumbQueue.length) {
        const id = String(WS.imageThumbQueue.shift() || "");
        if (!id) continue;
        WS.imageThumbQueuedIds.delete(id);
        if (WS.imageThumbInFlightIds.has(id)) continue;
        const rec = WS.fileById.get(id);
        if (!rec || rec.type !== "image") continue;
        ...
        WS.imageThumbInFlightIds.add(id);
        WS.imageThumbActive++;
        generateImageThumb(rec).catch(() => {}).finally(() => {
          WS.imageThumbInFlightIds.delete(id);
          WS.imageThumbActive--;
          renderPreviewPane(false);
          refreshDirectoryInlinePreviewThumbForRecord(rec);
          drainImageThumbQueue();
    ...
    async function generateImageThumb(rec) {
      const mode = WS.meta && WS.meta.options ? String(WS.meta.options.imageThumbSize || "medium") : "medium";
      if (await hydrateImageThumbFromDiskCache(rec, mode)) return;
      ...
      canvas.height = h;
      const ctx = canvas.getContext("2d");
      renderFilteredToCanvas(ctx, bmp, bmp.width || w, bmp.height || h, w, h, "off", true, rec);
      ...
      rec.thumbUrl = cachedUrl || URL.createObjectURL(blob);
      rec.thumbMode = mode;
    }
